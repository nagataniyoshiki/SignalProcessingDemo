...
fig1 = figure("Figure_name", "DFT Length Demo", "position", [0 0 1900 900], "BackgroundColor", [1 1 1]);
...
timeaxis = [0, -abs((amp1+amp2)*1.1); (Nmax-1)*dt*1000, abs((amp1+amp2)*1.1)];
freqaxis = [0,-90; fs, 50];
...
while Nmin <= N & N < Nmax
    ...
    subplot(2,1,2);
    set(gca(),"auto_clear","on");
    plot(f,20*log10(abs(fft(Sig_Hann))),'bo-');
    xgrid(1, 1, 3);
